...
def get_peft_model_state_dict(model, state_dict=None, adapter_name="default"):
    ...
    if config.peft_type in (PeftType.LORA, PeftType.MELORA, PeftType.ADALORA):
...
def set_peft_model_state_dict(model, peft_model_state_dict, adapter_name="default"):
    ...
    if getattr(model, "modules_to_save", None) is not None:
        for key, value in peft_model_state_dict.items():
            if any(module_name in key for module_name in model.modules_to_save) and 'original_module' not in key:
    ...
    if config.peft_type in (PeftType.MELORA):
