...
class MELoraConfig(PeftConfig):
    ...
    r: List[int] = field(default_factory=lambda: [2, 4, 6, 8], metadata={"help": "Lora attention dimension"})
    ...
    lora_alpha: List[int] = field(default_factory=lambda: [16, 16, 16, 16], metadata={"help": "Lora alpha"})
    ...
    mode: str = field(default="sum", metadata={
        "help": "Which mode to use for hierachical lora. "})
    ...
    def __post_init__(self):
        self.peft_type = PeftType.MELORA
...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, mode, init_lora_weights):
        self.r[adapter_name] = r
        self.lora_alpha[adapter_name] = lora_alpha
        self.mode = mode
        self.l_num = len(r)
        self.in_features//=self.l_num
        self.out_features//=self.l_num
        for i, rank in enumerate(r):
            if lora_dropout > 0.0:
                self.lora_dropout.append(nn.Dropout(p=lora_dropout))
            else:
                self.lora_dropout.append(nn.Identity())
            if rank > 0:
                self.lora_A.append(nn.Linear(self.in_features, rank, bias=False))
                self.lora_B.append(nn.Linear(rank, self.out_features, bias=False))
...
class MELoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: MELoraConfig) -> None:
        """
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(adapter_name, r, lora_alpha, lora_dropout, mode, init_lora_weights)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # if self.active_adapter not in self.lora_A_0.keys():
        #     return F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
        if self.disable_adapters:
            if self.r[self.active_adapter] > 0 and self.merged:
                self.unmerge()
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
        elif len(self.r[self.active_adapter]) > 0 and not self.merged:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)

            x = x.to(self.lora_A[0].weight.dtype)
            temp = []
            for i, rank in enumerate(self.r[self.active_adapter]):
                if rank > 0:
                    temp.append(self.lora_B[i](
                        self.lora_A[i](self.lora_dropout[i](x[:,:,i*(self.in_features):(i+1)*(self.in_features)]))
                        )
                        * self.scaling[i]
                    )
